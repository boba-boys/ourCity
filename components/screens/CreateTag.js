...
const CreateTag = () => {
  ...
  const pressedResult = useSelector(
    (state) => state.setPressedSearchResultsOnState
  );
  ...
  const onSearch = async () => {
    ...
    let results = await axios.get(
      `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${formattedSearch}&types=establishment&location=${coordinates.lat}%2C${coordinates.long}&radius=100000&strictbounds=true&key=${Constants.manifest.extra.API_KEY}`
    );

    const promisedKeys = results.data.predictions.map(async (place) => {
      return await axios.get(
        `https://maps.googleapis.com/maps/api/place/details/json?place_id=${place.place_id}&fields=name%2Crating%2Cformatted_phone_number%2Cformatted_address%2Cphotos%2Cgeometry&key=${Constants.manifest.extra.API_KEY}`
      );
    });
